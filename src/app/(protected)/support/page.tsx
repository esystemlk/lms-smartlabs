...
export default function SupportPage() {
  ...
  useEffect(() => {
    if (!loading && userData) {
      const allowedRoles = ["admin", "superadmin", "developer", "service"];
  ...
  useEffect(() => {
    const unsubscribe = chatService.subscribeToAllChats((updatedChats) => {
      ...
      // Simple notification check (if unreadByAdmin increased on any chat)
      // For now just console log or play sound
      const hasUnread = updatedChats.some(c => c.unreadByAdmin > 0);
      if (hasUnread) {
